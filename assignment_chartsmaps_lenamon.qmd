...
**3)**
For the Senate race, create a chart that shows the top 10 biggest drops between the Republican candidate and Trump (i.e. the 10 counties with the most substantial negative numbers in `pct_r_above_trump`). Remember that you can slice off a certain number of rows using the `head()` function, i.e. `head(5),` `head(20)`, etc. Your chart should be sorted from the biggest drop to the smallest among the top 10 counties.
...
```{r}
number7 <- merge(counties_geo, senate_results, by.x= "NAME", by.y = "county")
tm_shape(number7) + tm_polygons(col="pct_r_above_trump")
...
```{r}
number8 <- merge(counties_geo, governor_results, by.x= "NAME", by.y = "county")
tm_shape(number8) + tm_polygons(col="pct_r_above_trump")
...
number9 <- merge(counties_geo, margin, by.x= "NAME", by.y = "county")
tm_shape(number9) + tm_polygons(col="dem_margin")
...
number10 <- merge(counties_geo, gov_margin, by.x= "NAME", by.y = "county")
tm_shape(number10) + tm_polygons(col="dem_margin")
